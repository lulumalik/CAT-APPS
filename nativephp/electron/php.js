...
const requestedPhpVersion = phpVersion;
const fallbackVersions = [requestedPhpVersion, '8.4', '8.3'].filter(Boolean);
const phpVersionZip = fallbackVersions
    .map((version) => 'php-' + version + '.zip')
    .find((filename) => fs.existsSync(join(phpBinaryPath, platform.os, platform.arch, filename)));
const binarySrcDir = phpVersionZip
    ? join(phpBinaryPath, platform.os, platform.arch, phpVersionZip)
    : join(phpBinaryPath, platform.os, platform.arch, 'php-' + requestedPhpVersion + '.zip');
...
console.log('PHP version: ' + requestedPhpVersion + (phpVersionZip ? ` (using ${phpVersionZip.replace('php-', '').replace('.zip', '')})` : ''));
