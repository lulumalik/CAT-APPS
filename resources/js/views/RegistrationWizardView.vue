      ...
      <div class="bg-white rounded-[2rem] border border-gray-100 shadow-lg shadow-black/5 p-8 space-y-4">
        ...
        <div class="space-y-4">
          <template v-if="activeStep === 'administration'">
            ...
            <div v-show="showAutoGeneratedFormDownloads" class="hidden" aria-hidden="true">
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-5 space-y-4">
              <div>
                ...
                <p class="text-xs text-gray-500 mt-1">{{ t('registration.forms.templateHint') }}</p>
              </div>
              <a
                href="/api/my-registration/berkas-pdf"
            ...
            <div v-show="showAutoGeneratedFormDownloads" class="hidden space-y-4" aria-hidden="true">
            <div class="rounded-xl border border-gray-200 bg-white p-5 space-y-4">
              <div>
                <p class="font-medium text-gray-800">{{ t('registration.forms.sectionTitle') }}</p>
...
const formPages = ref([])
const showAutoGeneratedFormDownloads = false
...
async function fetchProgress() {
  ...
  try {
    ...
    progress.value = null
    loadErrorMessage.value = error?.response?.data?.message || 'Gagal memuat progress pendaftaran.'
  } finally {
    loading.value = false
  }
  if (progress.value) {
...
async function showFormError(message) {
  await showAlert({
    ...
    message: message || 'Periksa kembali isian Anda lalu coba simpan lagi.',
    confirmText: 'OK',
    type: 'danger',
  })
}
...
async function submitStep() {
  ...
  if (!isValidPhoneLocal(form.administration.whatsapp) || !isValidPhoneLocal(form.administration.phone)) {
    await showFormError('Format nomor WhatsApp atau telepon orang tua tidak valid. Gunakan format seperti 812345678.')
    return
  }
  saving.value = true
  try {
    ...
    await showFormError(extractErrorMessage(error, 'Gagal menyimpan data administrasi.'))
  } finally {
    saving.value = false
